...
class Tenant(Base):
    """One record per store registered in the system."""
    __tablename__ = "tenants"

    id           = Column(Integer, primary_key=True, autoincrement=True)
    store_name   = Column(String(255), nullable=False)      # e.g. "Toko Budi"
    owner_id     = Column(BigInteger, nullable=False)       # telegram_id of the owner
...
class BotUser(Base):
    ...
    Role hierarchy:
        owner  >  kasir
    Registration is done via the web app. Users activate their account in the
    Telegram bot by submitting the token they received from the website.
    """
    ...
    role               = Column(String(20), nullable=False)               # owner / kasir
    tenant_id          = Column(Integer, ForeignKey("tenants.id"), nullable=True)
    is_active          = Column(Boolean, default=True)
    added_by           = Column(BigInteger, nullable=True)                # telegram_id of registrar
    token_validated_at = Column(DateTime, nullable=True)                  # when they first activated via token
    created_at         = Column(DateTime, default=datetime.now)
    updated_at         = Column(DateTime, default=datetime.now, onupdate=datetime.now)
...
class RegistrationToken(Base):
    """
    One-time tokens generated by the web app (for owner) or by an owner in the
    Mini App (for kasir). A kasir uses this token in the Telegram bot to activate
    their account.

    Token format: POS-OWNER-<8 random chars> or POS-KASIR-<8 random chars>
    """
    ...
    role       = Column(String(20), nullable=False)               # owner / kasir
    tenant_id  = Column(Integer, ForeignKey("tenants.id"), nullable=True)   # set for kasir tokens
    created_by = Column(BigInteger, nullable=True)                # telegram_id of owner (for kasir tokens)
    is_used    = Column(Boolean, default=False)
    used_by    = Column(BigInteger, nullable=True)                # telegram_id that activated this token
    expires_at = Column(DateTime, nullable=False)                 # 48 hours default
    created_at = Column(DateTime, default=datetime.now)
...
class Transaction(Base):
    ...
    tenant_id links the transaction to the store it belongs to.
    """
    ...
    user_id = Column(BigInteger)
    tenant_id = Column(Integer, ForeignKey("tenants.id"), nullable=True)  # NEW: store context
